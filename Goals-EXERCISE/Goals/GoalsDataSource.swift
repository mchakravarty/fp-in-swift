...
class GoalsDataSource: NSObject {
  ...
  func configure(model: Changing<Goals>, edits: ChangesInlet<GoalEdit>) {
    // FIXME: EXERCISE: set 'goalsObservation' by observing 'model' (don't forget to reload the table view!)
    goalEdits = edits
  }
  ...
  func add(goal: Goal) {
    ...
    // FIXME: EXERCISE: announce edit without triggering a *local* observation
...
extension GoalsDataSource: UITableViewDataSource {
                 ...
                 forRowAt indexPath: IndexPath)
  {
    ...
    // FIXME: EXERCISE: announce deletion without triggering a *local* observation
